      ...
      embeds: [
        {
          description: `**Stats for <@${interaction.options.data[0]?.value}>**\n\n` +
            ...
            `**\`Valid & Fixed:\`** ${postData.filter(x=>x.solved === true).length} (${(postData.filter(x=>x.solved === true).length / postData.length) * 100}%)\n` +
            `**\`Invalid & Removed:\`** ${postData.filter(x=>x.solved !== true && x.removed === true).length} (${(postData.filter(x=>x.solved !== true && x.removed === true).length / postData.length) * 100}%)\n` +
